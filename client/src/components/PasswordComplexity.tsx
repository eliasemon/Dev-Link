...
import { PasswordSchema } from '../valibot/SchemaTypes'; // Assuming PasswordRule type exists in SchemaTypes
...
const PasswordComplexity: React.FC<PasswordComplexityProps> = ({
  ...
  if (!result.success) {
    result.issues.forEach((value) => {
      issues[value.expected] = value.expected;
    });
  }
  ...
  return (
    <div className="flex flex-col gap-1">
      {PasswordSchema.pipe.map((rule, index) => {
        if (rule.message === undefined) return null;
        return (
          <p
            key={index}
            className={`flex flex-row items-center text-sm ${
              checkRule(rule.expects) ? 'text-green-600' : 'text-red-600'
            }`}
          >
            {checkRule(rule.expects) ? (
              <svg
                width="25px"
                height="25px"
                viewBox="0 0 117 117"
